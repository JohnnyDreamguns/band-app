...
describe('getAlbumpageData', () => {
  it('calls the next function', async () => {
    const action = { type: 'TEST', id: 1 };
    await getAlbumpageData(store)(next)(action);
    expect(next).toHaveBeenCalledWith(action);
  });

  it('triggers sequence of api requests to endpoints getAlbumpageData and getBandById', async () => {
    const spy = sandbox
...
describe('getAlbumpageDataSuccess', () => {
  it('calls the next function', () => {
    const action = { type: 'TEST', payload: {} };
    getAlbumpageDataSuccess(store)(next)(action);
    expect(next).toHaveBeenCalledWith(action);
  });
...
describe('getAlbumpageDataError', () => {
  it('calls the next function', () => {
    const action = { type: 'TEST', payload: {} };
    getAlbumpageDataError(store)(next)(action);
    expect(next).toHaveBeenCalledWith(action);
  });
